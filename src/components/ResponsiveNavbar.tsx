...
const navLinks = [
  ...
  { name: 'Duo', href: '/duo', icon: <User size={16} /> },
  { name: 'Coaching', href: '/coaching', icon: <User size={16} /> },
  { name: 'Tournaments', href: '/tournaments', icon: <Gamepad2 size={16} /> },
];
...
export default function ResponsiveNavbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Change to 10px instead of 20px to make the solid header appear sooner
  ...
  return (
    <nav 
      className={`fixed w-full z-50 transition-all duration-300 ${
        scrolled 
          ? 'bg-[#0d0d0d]/95 backdrop-blur-sm py-3' // Reduced padding when scrolled
          : 'bg-[#0d0d0d]/80 py-4' // Semi-transparent background even at top
      }`}
      style={{ height: scrolled ? '70px' : '80px' }} // Consistent height
    >
